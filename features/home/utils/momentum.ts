...
/**
 * 参加予定数が多いだけで上位固定にならないように「log補正」を入れた勢いスコア。
 * - currentValue: 現在参加予定数など（goalType次第だが、基本は currentValue を使用）
 * - goalValue: 目標
 * - createdAt/updatedAt が無い前提でも動く（new24hは任意で渡せるようにしている）
 */

export type MomentumParams = {
  new24h?: number; // 24h以内の新規参加予定数（取れない場合は0でOK）
};
