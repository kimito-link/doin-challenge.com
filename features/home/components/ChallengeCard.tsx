...
export const ChallengeCard = memo(
      ...
      >
        <View style={{ position: "absolute", top: 8, left: 8, zIndex: 1 }}>
          <View style={{ backgroundColor: typeBadge.color, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4 }}>
            <Text style={{ color: colors.foreground, fontSize: homeFont.meta, fontWeight: "bold" }}>
              {typeBadge.label}
            </Text>
          </View>
        </View>
        ...
        >
          ...
          {challenge.venue && (
            <View style={{ position: "absolute", top: 8, left: 8, right: 32 }}>
              <Text
                style={{ color: "rgba(255,255,255,0.9)", fontSize: homeFont.small, fontWeight: "600" }}
                numberOfLines={1}
              >
                {challenge.venue}
              </Text>
          ...
          <View style={{ position: "absolute", bottom: -16, left: 12 }}>
            <LazyAvatar
              source={
                challenge.hostProfileImage && challenge.hostProfileImage.trim() !== ""
                  ? { uri: challenge.hostProfileImage }
                  : undefined
              }
              size={32}
        ...
        <View style={{ padding: 16, paddingTop: 20 }}>
          <Text
            style={{ color: colors.foreground, fontSize: homeFont.body, fontWeight: "bold", marginBottom: 4 }}
            numberOfLines={2}
          >
            {challenge.title}
          </Text>
          <Text style={{ color: homeText.muted, fontSize: homeFont.meta, marginBottom: 8 }} numberOfLines={1}>
            {challenge.hostName}
          </Text>
          <View style={{ marginBottom: 8 }}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
              <Text style={{ color: colors.foreground, fontSize: homeFont.title, fontWeight: "bold" }}>
                ...
                <Text style={{ fontSize: homeFont.meta, color: homeText.muted }}>
                  {" "}
                  / {challenge.goalValue}
                  {unit}
                </Text>
  ...
  // カスタム比較関数: IDと現在値が同じなら再レンダリングしない
  (prevProps, nextProps) => {
    return (
