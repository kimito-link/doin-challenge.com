...
export function useHomeData({ filter, searchQuery, categoryFilter }: UseHomeDataOptions): UseHomeDataReturn {
  ...
  const displayChallenges = useMemo(() => {
    const source = isSearching && searchResults ? searchResults : effectiveChallenges || [];
    return source.filter((c: Challenge & { categoryId?: number | null }) => {
  ...
  }, [isSearching, searchResults, effectiveChallenges, filter, categoryFilter, favorites]);
  ...
  const featuredChallenge = useMemo(() => {
    ...
    return effectiveChallenges.reduce((best, current) => {
      ...
      if (
        ...
        (currentProgress === bestProgress && current.currentValue > best.currentValue)
      ) {
        return current;
      }
      return best;
