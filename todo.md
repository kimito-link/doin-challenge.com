...
- [ ] diff機能による誤作動防止ワークフローを導入
  - [ ] ワークフロー文書を作成（docs/workflow-diff-check.md）
  - [ ] GitHub ActionsにPR時の自動チェックを追加
  - [ ] 既存機能のテストリストを作成

## P0（最優先）- お気に入り機能再実装（v6.54+）

- [ ] お気に入り機能を再実装（diff確認しながら、既存機能を壊さないように）
  - [ ] マイページに「気になるイベントリスト」セクションを追加
  - [ ] ログイン画面・サムネイル表示が壊れていないことを確認
