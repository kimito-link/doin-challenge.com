...
### 使用可能なアクセント色（限定）
- [ ] 主アクセント：#EC4899（ピンク）を定義
- [ ] 補助アクセント：#8B5CF6（紫）を定義
- [ ] 情報・中立強調：#0F766E（ティール）を定義
- [ ] 危険/警告のみ：赤（エラー専用）を維持

### 1. 応援コメントカード（性別ボーダー導入）【優先度1】
- [x] 左ボーダー2px追加（男性：青 #3B82F6、女性：ピンク #F472B6、未設定：ニュートラル）
- [x] 名前横に極小アイコン（♂ / ♀ / 無印）追加
- [x] 背景色・文字色は変えない
- [x] テスト追加（message-card-gender.test.ts）

### 2. イベント詳細ヘッダー（黒ベース＋白バッジ化）【優先度2】
- [x] テキスト色を黒系に変更（primary: #111827、secondary: rgba(17,24,39,0.78)、muted: rgba(17,24,39,0.62)）
- [x] バッジを白半透明背景に変更（bg: rgba(255,255,255,0.92)、text: #111827）
- [x] フォロワー数バッジ：白バッジ + 黒文字 + ティールアイコン
- [x] TwitterUserCardにlightBackground variantを追加
- [x] テスト追加（twitter-user-card-light-bg.test.ts）### 3. ログイン直後Welcome画面【優先度3】
- [x] 新規Welcome画面コンポーネント作成（RoleSelectionModal）
- [x] 選択肢：「ファンとして参加」「主催者として企画」
- [x] 説明文は短く、価値が即伝わる内容
- [x] オンボーディング後・ログイン直後に1回表示（初回ログイン判定）
- [x] テスト追加（role-selection-modal.test.ts）催者「次にやること」チェックリスト【優先度4】
- [ ] 既存のChallengeCreatedModalを確認・改善
- [ ] チェックリストの視認性向上
...
### セマンティックカラートークン追加
- [ ] theme/tokens/semantic.tsにlightBackground用トークン追加
- [ ] onLightPrimary: #111827
...
## v6.64 本番デプロイ（2026-01-23）

- [x] v6.63 UI/UX視認性改善をGitHubにpush
- [x] v6.64 売れる構造の強化をGitHubにpush
- [x] GitHub Actionsでデプロイトリガー
...
### 問題1: ベースカラーが黒ではない
- [ ] 現在のテーマ設定を確認
- [ ] background色を純粋な黒（#000000または#0a0a0a）に変更
- [ ] 青みがかったダークカラーを排除
...
## v6.65 仕様確定メモ準拠：全体カラー方針の正しい実装

### 全体カラー方針（仕様確定メモより）
- [x] ベースカラーを黒に変更（#0a0a0a）
- [x] 黄色を完全に廃止（全てピンクに置換）
- [x] アクセント色をピンク#EC4899、紫#8B5CF6、ティール#0F766Eのみに限定
- [x] 背景塗り・文字色にアクセント色を使わない（アイコン・ボーダー・左線のみ）

### palette.ts書き換え
- [x] gray900を#0a0a0aに変更
- [x] 黄色系（accent500, accent600, yellow500等）を廃止→ピンクに置換
- [x] primary500をピンク#EC4899に変更
- [x] 紫#8B5CF6をaccent500として追加
- [x] ティール#0F766Eをteal500として追加

### 黄色使用箇所の置換
- [x] 全ファイルで黄色（#FFB703, amber, yellow）を検索
- [x] ui-components.test.tsを仕様準拠に更新
- [x] twitter-user-card-light-bg.test.tsのコメントを更新
...
### E2Eテストエラー修正
- [ ] localhost:8081問題を修正
- [ ] プロフィール「見つかりません」問題を修正


## v6.65: アクセシビリティ修正

- [x] セカンダリテキストのコントラスト比修正（#737373 → #7a7a7a、4.61:1）
- [x] WCAG AA準拠（4.5:1以上）を達成
- [x] ChatGPT相談用ドキュメント作成（docs/chatgpt-consultation.md）
