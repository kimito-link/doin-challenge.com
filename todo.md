...
### 真っ暗画面問題の修正後の課題
- [ ] チュートリアルが表示されない（TutorialUIが`return null`で無効化中）
- [ ] 追体験をクリックしても反応しない（ExperienceOverlayがコメントアウト中）
- [ ] チャレンジ作成後に一覧に表示されない
- [ ] 作成後の確認画面が表示されない
...
### 追体験の操作性改善
- [ ] 画面の外（オーバーレイ部分）をタップしたら次に進む
- [ ] 「次へ」「戻る」ボタンの位置を調整

- [ ] ログアウト画面の視認性向上（ボタンの色、コントラスト）


## 緊急修正（v6.10）

### サイトが真っ白になる問題
- [ ] ExperienceOverlayの修正でエラーが発生 → 元に戻す

### 主催者のTwitterサムネイル表示問題
- [ ] 一部のチャレンジカードで主催者のアイコンが表示されていない


### イベント詳細画面の視認性問題
- [ ] 「フォロワー」テキストが薄くて見えにくい
- [ ] プロフィール説明テキストのコントラストが低い
- [ ] 全体的なテキストの視認性を向上
...
### 追体験のUI/UX改善
- [ ] 追体験のUI/UXをチュートリアルと同じにする（使いやすいので）
- [ ] 画面タップで次に進む機能
- [ ] ボタンの位置・デザインをチュートリアルと統一
...
### 視認性の問題（継続）
- [ ] フォロワー数がまだ薄い → もっと明るく（白に近い色に）

### ナビゲーション問題
- [ ] イベント詳細画面に遷移後、一瞬スケルトン表示されてメインページに戻ってしまう


## バグ修正（v6.13）

### チャレンジ作成画面の問題
- [x] ログインユーザーのプロフィール文章（description）が表示されていない → 修正済み


## 調査・確認（v6.14）

### スケルトン表示問題
- [ ] イベント詳細画面に飛んだ後、スケルトン表示→メインページに戻る問題を調査

### 編集・削除機能の確認
- [ ] 主催者：チャレンジの編集・削除機能の現状確認
- [ ] ファン：参加表明の編集・削除機能の現状確認


## UI改善（v6.14）

### プロフィール文章のレスポンシブ対応
- [ ] TwitterUserCardのプロフィール文章が右端で切れている → 複数行で表示するように修正
