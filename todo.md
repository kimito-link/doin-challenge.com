...
## P0（最優先）- ログインフローUX改善（v6.55+）

- [ ] Twitter/Xログインフローを「りんく」のオリジナル体験に統一
  - [ ] 現在のログインフローのコードを調査
  -- [x] 共通コンポーネントを作成（LinkSpeech/LoginConfirmModal）
- [x] ログイン確認モーダルを実装
- [x] マイページ未ログイン画面を修正（ボタン文言・絵文字削除）画面を修正（ボタン文言「Xでログインする」）
  - [ ] キャンセル画面を実装（「ログインをキャンセルしました」）
