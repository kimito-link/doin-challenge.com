...
var init_challenges = __esm({
  "drizzle/schema/challenges.ts"() {
    ...
    challenges = mysqlTable2("challenges", {
      ...
      imageUrl: text2("imageUrl"),
      thumbnailUrl: text2("thumbnailUrl"),
...
var init_challenge_db = __esm({
  "server/db/challenge-db.ts"() {
    ...
    safeEventColumns = {
      ...
      imageUrl: events2.imageUrl,
      thumbnailUrl: events2.thumbnailUrl,
      status: events2.status,
      isPublic: events2.isPublic,
      createdAt: events2.createdAt,
      updatedAt: events2.updatedAt,
...
var notificationsRouter = router({
  ...
  // プッシュ通知トークンを登録
  registerPushToken: protectedProcedure.input(
    z4.object({
      token: z4.string(),
      platform: z4.enum(["ios", "android", "web"])
    })
  ).mutation(async ({ input, ctx }) => {
    return { success: true };
  }),
...
async function startServer() {
  ...
  app.get("/api/health", async (_req, res) => {
    try {
      ...
      if (!buildInfo.ok) {
        console.error("unknown version in /api/health", {
