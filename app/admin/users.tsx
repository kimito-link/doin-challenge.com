          ...
          <View className="gap-3">
            {users.map((user) => (
              <Pressable
                ...
                accessibilityLabel={`${user.name || user.email || user.openId} ${user.role === 'admin' ? '管理者' : ''}`}
                accessibilityState={{ expanded: selectedUser?.id === user.id }}
                        ...
                        <View className="flex-row gap-2 mt-4">
                          {user.role === "user" ? (
                            <Pressable
                              ...
                              accessibilityRole="button"
                              accessibilityLabel="管理者に昇格"
                              accessibilityState={{ disabled: updateRoleMutation.isPending }}
                              style={({ pressed }) => [
