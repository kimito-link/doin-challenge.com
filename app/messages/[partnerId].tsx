...
export default function ConversationScreen() {
    ...
    return (
      <ScreenContainer className="p-6">
        <View className="flex-1 items-center justify-center">
          <Text className="text-lg text-muted text-center">メッセージを送るにはログインが必要です</Text>
        </View>
      </ScreenContainer>
    );
  }
  ...
  const renderMessage = ({ item }: { item: NonNullable<typeof messages>[0] }) => {
    ...
    return (
      <View className={`flex-row mb-3 ${isMe ? "justify-end" : "justify-start"}`}>
        <View className={`max-w-[75%] rounded-2xl px-4 py-3 ${isMe ? "bg-primary" : "bg-surface"}`}>
          <Text className={`text-base ${isMe ? "text-background" : "text-foreground"}`}>{item.message}</Text>
          <Text className={`text-xs mt-1 ${isMe ? "text-background/70" : "text-muted"}`}>
  ...
  return (
    <ScreenContainer edges={["top", "left", "right"]}>
      <KeyboardAvoidingContainer className="flex-1" keyboardVerticalOffset={Platform.OS === "ios" ? 90 : 0}>
        {/* ヘッダー */}
        <AppHeader
          title="君斗りんくの動員ちゃれんじ"
        ...
        <View className="p-4 border-b border-border">
          <Text className="text-lg font-bold text-foreground">メッセージ</Text>
        </View>
        ...
        <View className="flex-row items-end p-4 border-t border-border bg-background">
          ...
          >
            <Text className={`text-xl ${message.trim() ? "text-background" : "text-muted"}`}>↑</Text>
          </Pressable>
        </View>
