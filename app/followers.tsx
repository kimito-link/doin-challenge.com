...
export default function FollowersScreen() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.follows.followers.useQuery({ userId: targetUserId || 0 }, { enabled: !!targetUserId });
  ...
  return (
    <ScreenContainer containerClassName="bg-background">
      ...
      <AppHeader
        ...
        rightElement={
          <Pressable onPress={() => navigate.back()} style={{ flexDirection: "row", alignItems: "center" }}>
      ...
      >
        <Text style={{ color: color.textWhite, fontSize: 18, fontWeight: "bold" }}>フォロワー</Text>
      </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              {item.followerImage ? (
                <Image source={{ uri: item.followerImage }} style={{ width: 48, height: 48, borderRadius: 24 }} />
              ) : (
                <View
                  style={{
                    width: 48,
