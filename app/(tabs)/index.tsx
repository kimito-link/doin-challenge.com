...
import { ProfilerWrapper } from "@/lib/profiler-wrapper";
...
export default function HomeScreen() {
  ...
  // FlatList最適化設定
  const { optimizedProps } = useOptimizedFlatList(isSearching ? FLATLIST_PRESETS.grid : FLATLIST_PRESETS.standard);
  ...
  usePerformanceMonitor("Home", homeData.hasData, homeData.isInitialLoading, !homeData.hasData);
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: Challenge; index: number }) => {
      if (isSearching) {
        ...
        return useColorfulCards ? <ColorfulChallengeCard {...cardProps} /> : <ChallengeCard {...cardProps} />;
      }
      return (
    ...
    [isSearching, useColorfulCards, numColumns, grid.itemWidth, user?.twitterId, homeActions, homeData]
  );

  return (
    <ProfilerWrapper id="HomeScreen">
      <ScreenContainer containerClassName="bg-background">
        ...
        {homeData.hasError && !homeData.hasData ? (
          ...
          <FlatList
            ...
            refreshControl={<SimpleRefreshControl refreshing={homeData.refreshing} onRefresh={homeData.onRefresh} />}
            onEndReached={() => {
              if (isSearching) {
            ...
            columnWrapperStyle={
              isSearching && numColumns > 1 ? { justifyContent: "flex-start", gap: grid.gap } : undefined
            }
            {...optimizedProps}
          />
        ) : (
        ...
        {/* FABボタン */}
        <FloatingActionButton onPress={homeActions.handleCreateChallenge} icon="add" size="large" />
