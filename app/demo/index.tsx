...
export default function DemoScreen() {
  ...
  const handleJoin = async () => {
    ...
    // ボタンアニメーション
    buttonScale.value = withSequence(withTiming(0.9, { duration: 100 }), withSpring(1, { damping: 10 }));

    try {
      const newState = await joinDemoChallenge();

      // 数字のアニメーション
      numberScale.value = withSequence(withTiming(1.3, { duration: 200 }), withSpring(1, { damping: 8 }));
  ...
  const handleAddContribution = async () => {
    ...
    try {
      ...
      // 数字のアニメーション
      numberScale.value = withSequence(withTiming(1.2, { duration: 150 }), withSpring(1, { damping: 10 }));
  ...
  return (
    <ScreenContainer>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16 }}>
        {/* デモバナー */}
        <View className="mb-4 p-3 rounded-xl flex-row items-center" style={{ backgroundColor: colors.warning + "20" }}>
        ...
        <View className="bg-surface rounded-2xl p-5 border border-border mb-4">
          ...
          <View className="mb-4">
            <View className="flex-row items-end justify-between mb-2">
              <Animated.Text className="text-3xl font-bold text-foreground" style={numberAnimatedStyle}>
                {challenge.currentValue}
              </Animated.Text>
              <Text className="text-lg text-muted">/ {challenge.goalValue}人</Text>
            </View>
            <View className="h-4 rounded-full overflow-hidden" style={{ backgroundColor: colors.border }}>
              <Animated.View
                className="h-full rounded-full"
                style={[{ backgroundColor: colors.primary }, progressAnimatedStyle]}
              />
            </View>
            <Text className="text-sm text-muted mt-1 text-right">達成率 {progress.toFixed(1)}%（参加予定）</Text>
          </View>
        ...
        {!challenge.userState.hasJoined ? (
            ...
            >
              {joining ? (
                ...
                <View className="flex-row items-center">
                  ...
                  <Text className="text-white text-lg font-bold ml-2">参加する（お試し）</Text>
                </View>
              )}
            </Pressable>
          </Animated.View>
          ...
          <View>
            {/* 参加済み表示 */}
            <View className="p-4 rounded-xl mb-4 items-center" style={{ backgroundColor: colors.success + "20" }}>
              <View className="flex-row items-center">
              ...
              <Text className="text-sm text-muted mt-1">あなたの貢献: {challenge.userState.contribution}人</Text>
            </View>
        ...
        <Animated.View
          ...
          pointerEvents="none"
        >
          <View className="px-6 py-4 rounded-2xl" style={{ backgroundColor: colors.primary }}>
            <Text className="text-white text-xl font-bold text-center">🎉 参加ありがとう！</Text>
            <Text className="text-white text-center mt-1">一緒に目標を達成しよう！</Text>
          </View>
        </Animated.View>

        {/* 参加者リスト */}
        <View className="mt-6">
          <Text className="text-lg font-bold text-foreground mb-3">参加者 ({challenge.participants.length}人)</Text>
              ...
              >
                ...
                <View className="flex-1 ml-3">
                  <Text className="font-medium text-foreground">
                    {participant.name}
                    {participant.id === "demo_user" && <Text style={{ color: colors.primary }}> (あなた)</Text>}
                  </Text>
                  <Text className="text-xs text-muted">貢献: {participant.contribution}人</Text>
                </View>
              </View>
            ))}
          </View>
        ...
        <View className="mt-8 p-5 bg-surface rounded-2xl border border-border">
          <Text className="text-lg font-bold text-foreground text-center mb-2">気に入りましたか？</Text>
          ...
          <Pressable
            onPress={handleLogin}
            accessibilityRole="button"
            accessibilityLabel="ログインして実際のチャレンジに参加する"
            style={({ pressed }) => ({
              backgroundColor: colors.primary,
              paddingVertical: 14,
              borderRadius: 12,
          ...
          >
            <View className="flex-row items-center">
              ...
              <Text className="text-white font-bold ml-2">ログインして始める</Text>
            </View>
          </Pressable>
        </View>
