...
import { speakSuccess } from "@/lib/accessibility-speech";
...
export function ChallengeCreatedModal({
  ...
  useEffect(() => {
    if (visible) {
      speakSuccess("チャレンジの作成が完了しました。おめでとうございます！");
    }
  }, [visible]);
  ...
  return (
    ...
    >
      <View style={[styles.overlay, { backgroundColor: palette.black + "CC" }]}>
        <View style={[styles.container, { backgroundColor: colors.background }]} accessibilityViewIsModal={visible} importantForAccessibility="yes">
          {/* ヘッダー */}
