...
export function RegionParticipantsModal({
  ...
  const renderPrefectureItem = ({ item }: { item: string }) => {
    ...
    return (
      <Pressable
        ...
        accessibilityLabel={`${item}, ${count}人`}
  ...
  const renderParticipant = ({ item }: { item: Participant }) => (
    <Pressable
      ...
      accessibilityRole="button"
      accessibilityLabel={`${item.isAnonymous ? "匿名ユーザー" : item.displayName}, ${item.contribution}人`}
      accessibilityHint={item.userId && !item.isAnonymous ? "タップしてプロフィールを表示" : undefined}
      disabled={!item.userId || item.isAnonymous}
      style={({ pressed }) => [
